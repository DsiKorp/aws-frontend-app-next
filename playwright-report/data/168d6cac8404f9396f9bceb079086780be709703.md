...
- Name: compare-yourself.spec.ts >> Compare Yourself >> shows Sign In and Sign Up links when not authenticated
- Location: e2e\compare-yourself.spec.ts:4:7
...
Locator: getByRole('navigation').getByRole('link', { name: 'Sign In' })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('navigation').getByRole('link', { name: 'Sign In' })
